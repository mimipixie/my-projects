...
if (!empty($_POST)) {
  ...
  if (empty($_POST["sid4"])) {
	...
	if($result->num_rows > 0 ) {
		$sidErr1= "Student Id is in use. Try new one";
		$gotoEm=1;
	} 
  }
...
if($gotoEm==0){
$gender=$_POST["gender"];
$insert=mysqli_query($conn,"insert into liststd
						(sid1 ,sid2 ,sid3,sid4,name,email,gender,addr,validity)
						values('$sid1','$sid2','$sid3','$sid4','$name','$email','$gender','$addr',1)");
  if($insert) {
...
?>
  ...
  <br>
                  <input type="text" name="sid4" maxlength="3" pattern="[0-9]{3}" size="3" title="3 digit code">
