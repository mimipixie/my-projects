...
$userid=mysqli_query($conn,"select username from student where username='$id'");
if($userid->num_rows > 0){
	$userpw=mysqli_query($conn,"select password from student where password='$sid'
							and  username='$id' ");
	if($userpw->num_rows > 0){
		$userv=mysqli_query($conn,"select * from student where password='$sid'
								and  username='$id' and validity=1 ");
		if($userv->num_rows > 0){
		$update=mysqli_query($conn,"update student set status=1 where username='$id'");
		...
		if(!empty($_POST["remember"])){
			...
			setcookie("usertype","student",(time()+(365*24*60*60)) );
		}else{
		...
		if(isset($_SESSION["username"])){
			...
			$idErr2 = "Student is not valid now";
		}
	}
	else{
