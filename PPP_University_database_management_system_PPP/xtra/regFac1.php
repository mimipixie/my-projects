...
$sql="select * from listsec where seats>0 and dcode='$dept' ";
$res=mysqli_query($conn,$sql);
if($res->num_rows >0){
	$ct=0;
	echo "<table>";
	...
	while(($ob=mysqli_fetch_object($res))){
		...
		echo "<tr>";
			...
			echo "<td> $ss </td>";
			echo "<td>"; if($gooo==0){?> <a href="<?php echo $str;?>">Register</a>	 
			<?php}else if($gooo==1){ echo "Registered";}else if($gooo==2){ echo "Taken";} 
			echo "</td>";
		echo "</tr>";
	}
