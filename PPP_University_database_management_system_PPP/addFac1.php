...
if (!empty($_POST)) {
  ...
  if (empty($_POST["id"])) {
	...
	$result=mysqli_query($conn,"select username from faculty where username='$id'");
  ...
  if (empty($_POST["sid4"])) {
	...
	if($result->num_rows > 0 ) {
		$result=mysqli_query($conn,"select * from faculty where sid1='$sid1' and 
					 sid2='$sid2' and  sid3='$sid3' and  sid4='$sid4' ");
		if($result->num_rows > 0 ) {
			$sidErr2= "Id is in use. Try different Id";
			$gotoEm=1;
		}
	} else{
		$sidErr1= "Invalid Id. Try valid one";
		$gotoEm=1;
	}
  }
...
$dept=$ob->code;
$insert=mysqli_query($conn,"insert into faculty
...
<div class="form">
<h2>Faculty Member Registration</h2> <br>
