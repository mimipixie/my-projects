...
$userid=mysqli_query($conn,"select username from admin where username='$id'");
if($userid->num_rows > 0){
	$userpw=mysqli_query($conn,"select password from admin where password='$sid'
							and  username='$id' ");
	if($userpw->num_rows > 0){
		$userv=mysqli_query($conn,"select * from admin where password='$sid'
								and  username='$id' and validity=1 ");
		if($userv->num_rows > 0){
		$update=mysqli_query($conn,"update admin set status=1 where username='$id'");
		...
		if(!empty($_POST["remember"])){
			...
			setcookie("usertype","admin",(time()+(365*24*60*60)) );
		}else{
